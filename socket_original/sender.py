    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            try:
                ...
                pass
            print(f"[INFO] {time_now()} - Image sent successfully to {host}:{port} | uid={uid_value} | bytes={encrypted_size} | attempt={attempt}")
            return 0
...
def parse_args():
    ...
    parser.add_argument('-i', '--image', dest='image', default=DEFAULT_IMAGE, help='Image file path to send')
    ...
    parser.add_argument('-n', '--uid', dest='uid', default=DEFAULT_UID, help='Device UID (<=32 bytes)')
    parser.add_argument('--retries', dest='retries', type=int, default=0, help='Retry attempts on failure')
    parser.add_argument('--test', dest='test', action='store_true', help='Send a generated 1x1 white PNG (overrides -i)')
